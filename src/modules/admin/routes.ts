...
const adPromoteSchema = z.object({
  durationDays: z.coerce.number().int().min(1).max(365).optional(),
  priority: z.coerce.number().int().min(1).max(100).optional(),
});

const adPromotionPrioritySchema = z.object({
...
router.patch("/ads/:id/promote", async (req: Request, res: Response) => {
  try {
    const id = String(req.params.id);
    const body = parseOrThrow(adPromoteSchema, req.body ?? {});
    const existing = await prisma.ad.findUnique({
      where: { id },
    ...
    invalidateCache("/admin/ads", "/admin/reports", "/admin/stats", "/ads", "/categories", "/users");
...
router.patch("/ads/:id/unpromote", async (req: Request, res: Response) => {
  try {
    const id = String(req.params.id);
    const existing = await prisma.ad.findUnique({
      where: { id },
...
router.patch("/ads/:id/promotion-priority", async (req: Request, res: Response) => {
  try {
    const id = String(req.params.id);
    const body = parseOrThrow(adPromotionPrioritySchema, req.body ?? {});
    const existing = await prisma.ad.findUnique({
      where: { id },
